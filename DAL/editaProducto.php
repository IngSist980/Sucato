...
$imagenNombre = recoge('imagenNombre');
...
if($idProducto == ""){
    print "<p class=\"aviso\">El código del producto no es válido</p>\n";
    print "\n";
}elseif(!is_numeric($idProducto)){
    print "<p class=\"aviso\">El código del producto no es válido</p>\n";
    print "\n";
}else {
    $idProductoOk = true;
...
if (isset($_FILES['imagenArchivo']) && $_FILES['imagenArchivo']['error'] == UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($tmp_name, $uploads_dir . $unique_name)) {
        ...
        if($idProductoOk && $nombreOk && $descripcionOk && $precioOk && $pesoOk && $imagenOk && $existenciasOk && $activoOk){
            include 'conexion.php';
            echo json_encode(actualizaDatos($idProducto, $nombre, $descripcion, $peso, $precio, $existencias, $unique_name, $activo));  
        }
        exit;
    } else {
    ...
    if($idProductoOk && $nombreOk && $descripcionOk && $precioOk && $pesoOk && $imagenNombreOk && $existenciasOk && $activoOk){
        include 'conexion.php';
        echo json_encode(actualizaDatos($idProducto, $nombre, $descripcion, $peso, $precio, $existencias, $imagenNombre, $activo));  
    }else{
        error_log('Error al actualizar producto');
    }
}
